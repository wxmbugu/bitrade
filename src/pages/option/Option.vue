                  ...
                  <div class="panel panel_buy">
                    <div class="bd bd_limited">
                      <div class="result-panel">
                        ...
                        <div class="history-tab" v-show="historyTab==1">
                      ...
                      <div class="last-period isdesktop" v-if="openingOption.id != 0">
                       ...
                       <div class="last-period ismobile" v-if="openingOption.id != 0">
                        ...
                        <div class="period-content ">
                          ...
                          <div  v-if="!isLogin">{{$t('option.myamount')}}: 
                              <router-link to="/login" id="login">{{$t("common.login")}}</router-link>/<router-link to="/register" id="register">{{$t("common.register")}}</router-link>
                          </div>
                        </div>

                        <div style="height:40px;line-height:40px;padding-right:10px;">
                          <span :class="{ 'bg-green': currentPrice > openPrice, 'bg-red': currentPrice < openPrice }" style="letter-spacing:2px;display: inline-block;height:20px;line-height:20px;border-radius: 3px;padding: 0 5px;color:#FFF;">{{priceChange}}</span>
                        </div>
                        <div style="color:#a5a5a5;width:170px;height:40px;text-align: center;">
                          <div style="height:16px;line-height: 16px;margin-top: 4px;margin-left:5px;font-size: 10px;text-align: left;">{{$t('option.openprice')}}：{{openPrice | fixed2}} USDT</div>
                          <div style="height:16px;line-height: 16px;font-size: 10px;margin-left:5px;text-align: left;">{{$t('option.currentprice')}}：<span :class="{ 'color_green': currentPrice > openPrice, 'color_red': currentPrice < openPrice }">{{currentPrice | fixed2}} USDT</span>
                          </div>
                        </div>

                        <div  style="width:80%;height:40px;line-height: 36px;text-align: right;padding-right: 10px;">
                            <div style="color:#ff8100;width:80px;height:40px;line-height: 40px;text-align: right;font-size:12px;">{{$t('option.progress')}}：</div>
                          <div><Progress :percent="currentPercent" :stroke-width="15" status="active"/></div>
                        </div>
...
export default {
  ...
  methods: {
    ...
    async getHistory(){
     ...
     await this.$http.post(this.host + "/option/option/history", params).then(response => {
        var resp = response.body;
        if (resp.code == 0) {
...
@media only screen and (max-width:768px) {
  ...
  .last-period{
   ...
   .last-period .period-content .content-item{
     width: 100% !important;
   }
   .option .trade_bd_ctc{
...
.option-tab-current{
  border-bottom: 2px solid #ffec03!important;
